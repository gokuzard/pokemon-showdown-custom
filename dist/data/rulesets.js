      ...
      for (const set of team) {
        ...
        if (set.item && this.dex.items.get(set.item).megaStone) {
          ...
          if (item.megaEvolves?.includes(species.name)) {
            species = this.dex.species.get(Array.isArray(item.megaEvolves) ? item.megaStone[item.megaEvolves.indexOf(species.name)] : item.megaStone);
          }
        }
      ...
      const god = target.side.team.find((set) => {
        ...
        if (set.item) {
          ...
          if (item.megaEvolves?.includes(set.species)) {
            godSpecies2 = this.dex.species.get(Array.isArray(item.megaEvolves) ? item.megaStone[item.megaEvolves.indexOf(set.species)] : item.megaStone);
          }
